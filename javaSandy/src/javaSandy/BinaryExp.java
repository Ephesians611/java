...
/**
 * An abstract class to characterize the common data and methods associated with
 * binary expressions.
 *
...
public abstract class BinaryExp implements Expression {
	...
	/**
	 * Initialize the two operands. This constructor can only be invoked by a
	 * sub-class.
	 *
	 * @param exp1
	 *            the Expression that is the first operand of the binary expression
	 * @param exp2
	 *            the Expression that is the second operand of the binary expression
	 */
	...
	@Override
	public ANFExp toANF() {
		Triple<ANFVarExp, ANFOp, Expression> var = extract();
		return new ANFLetExp(var.first(), var.second(), var.third().toANF());
	}
