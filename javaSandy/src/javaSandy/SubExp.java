...
public class SubExp extends BinaryExp {
	 ...
	 *
	 * @param exp1
	 *            the Expression that is the first operand of the subtraction
	 *            expression
	 * @param exp2
	 *            the Expression that is the second operand of the subtraction
	 *            expression
	 */
	...
	public Triple<ANFVarExp, ANFOp, Expression> extract() {
		if (getExp1() instanceof Holder && getExp2() instanceof Holder) {
			...
			return new Triple<>(t, new ANFSubOp(((Holder) getExp1()).getVar(), ((Holder) getExp2()).getVar()),
					new Holder(t));
