...
public class AddExp extends BinaryExp {
	 ...
	 *
	 * @param exp1
	 *            the Expression that is the first operand of the addition
	 *            expression
	 * @param exp2
	 *            the Expression that is the second operand of the addition
	 *            expression
	 */
	...
	public Triple<ANFVarExp, ANFOp, Expression> extract() {
		if (getExp1() instanceof Holder && getExp2() instanceof Holder) {
			...
			return new Triple<>(t, new ANFAddOp(((Holder) getExp1()).getVar(), ((Holder) getExp2()).getVar()),
					new Holder(t));
		} else if (getExp1() instanceof Holder) {
			Triple<ANFVarExp, ANFOp, Expression> temp = getExp2().extract();
			return new Triple<>(temp.first(), temp.second(), new AddExp(getExp1(), temp.third()));
		} else {
			Triple<ANFVarExp, ANFOp, Expression> temp1 = getExp1().extract();
			return new Triple<>(temp1.first(), temp1.second(), new AddExp(temp1.third(), getExp2()));
		}
